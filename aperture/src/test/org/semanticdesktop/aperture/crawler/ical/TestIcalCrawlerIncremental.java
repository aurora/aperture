...
public class TestIcalCrawlerIncremental extends ApertureTestBase {
	...
	public void testOneLetterChangedInTimezone() throws Exception {
		...
		// the timezone has nod UID, so it's uri is based on a hash of properties
		// we changed a property, so the uri has changed. That's why we have one new object and
		// one deleted object. Other four objects are unchanged
		assertNewModUnmodDel(handler2,1,0,4,1);
	}
	...
	public void printAccessDataContents() {
		System.out.println(accessData.toString());
		System.out.println();
	}
