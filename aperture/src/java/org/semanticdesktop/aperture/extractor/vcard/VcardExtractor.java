 ...
 * </ul>
 */
public class VcardExtractor implements Extractor {
	...
	public void extract(URI id, InputStream stream, Charset charset, String mimeType, RDFContainer result)
	    ...
	    try {
    	    ...
    	    if (contacts.length == 0) {
    	        ...
    	        processContact(contacts[0],result.getModel(),result.getDescribedUri());
    	    } else {
    	        processAddressBook(contacts,result);
    	    }
	    } catch (Exception e) {
	...
	private void processAddressBook(Contact[] contacts, RDFContainer result) {
        result.add(RDF.type, NCO.ContactList);
        for (Contact contact : contacts) {
            Resource contactResource = UriUtil.generateRandomResource(result.getModel());
            processContact(contact,result.getModel(),contactResource);
            result.add(NCO.containsContact,contactResource);
            result.getModel().addStatement(contactResource,RDF.type, NCO.ContactListDataObject);
        }
    }
    ...
    private void processContact(Contact contact, Model model, Resource contactResource) {
        ...
        Resource affiliationResource = processOrganizationIdentity(contact.getOrganizationalIdentity(), model, contactResource);
    ...
    private void processPersonalIdentity(PersonalIdentity personalIdentity, Model model,
        ...
        if (personalIdentity.getAdditionalNameCount() > 0 ||
            personalIdentity.getBirthDate() != null ||
            personalIdentity.getFirstname() != null ||
            personalIdentity.getLastname() != null ||
            personalIdentity.getNicknameCount() > 0 ||
            personalIdentity.listPrefixes().length > 0 ||
    ...
    private void processCommonProperties(Contact contact, Model model, Resource contactResource, Resource affiliationResource) {
        ...
        processGeographicalInformation(model, contactResource, NCO.hasLocation, contact.getGeographicalInformation());
    ...
    private void processAddress(Model model, Address address, Resource contactResource,
        ...
        if (address != null) {
            ...
            /*
             * First a set of flags that occur only on addresses of organizations. The problem is:
             * should we attache them to the contact (meaning that the contact itself is an OrganizationContact,
             * or to the Affiliation - which means a work address or to the personContact.
             * 
            ...
            if (address.isDomestic()) {
                ...
                addContactMediumProperty(model, contactResource, NCO.hasPostalAddress, addressResource, preferred);
            }
            ...
            if (address.isWork()) {
                if (affiliationResource != null) {
                    ...
                    addContactMediumProperty(model, affiliationResource, NCO.hasPostalAddress, addressResource, preferred);
                } else {
    ...
    private void processPhoneNumber(Model model, PhoneNumber number, Resource contactResource,
        ...
        if (number != null) {
            ...
            if (number.isHome()) {
                addContactMediumProperty(model, contactResource, NCO.hasPhoneNumber, numberResource, number.isPreferred());
            }
            if (number.isISDN()) {
            ...
            if (number.isWork()) {
                if (affiliationResource != null) {
                    // this means that an affiliation has already been created while processing
                    // the organizational identity
                    addContactMediumProperty(model, affiliationResource, NCO.hasPhoneNumber, numberResource, number.isPreferred());
                } else {
                    // this means that no affiliation has been created, we need to create one now
                    // an anonymous affiliation with an anonymous organization
                    affiliationResource = UriUtil.generateRandomResource(model);
    ...
    private void processEmailAddress(Model model, EmailAddress address, Resource contactResource,
            Resource affiliationResource, boolean preferred) {
        /*
         * This sucks, there is no way to tell if an email is a private email or a business email
         * this sucks ass...
         */
    ...
    private void processGeographicalInformation(Model model, Resource contactResource, URI property, GeographicalInformation geo) {
        if (geo != null) {
