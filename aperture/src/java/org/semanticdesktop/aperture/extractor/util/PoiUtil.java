...
public class PoiUtil {
    ...
    public static InputStream extractMetadata(InputStream stream, boolean resetStream, RDFContainer container)
        ...
        POIFSFileSystem fileSystem = new POIFSFileSystem(new NonCloseableStream(stream));
     ...
     *         BufferedInputStream is returned
     */
    public static InputStream extractAll(InputStream stream, TextExtractor textExtractor,
            RDFContainer container, Logger logger) {
        ...
        // Only do this when the document is not encrypted though. 
        if (text == null && !encrypted) {
            if (textExtractor != null) {
                logger
                        .info("regular POI-based processing failed, falling back to heuristic string extraction for "
