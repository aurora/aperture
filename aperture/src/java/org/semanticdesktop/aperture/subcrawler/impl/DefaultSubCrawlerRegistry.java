 ...
 */
public class DefaultSubCrawlerRegistry extends SubCrawlerRegistryImpl {

    private static final String DEFAULT_FILE = "org/semanticdesktop/aperture/subcrawler/impl/defaults.xml";

    private static final String SUB_CRAWLER_FACTORY_TAG = "subCrawlerFactory";

    private static final String NAME_TAG = "name";

    public DefaultSubCrawlerRegistry() {
        try {
            InputStream stream = ResourceUtil.getInputStream(DEFAULT_FILE, DefaultSubCrawlerRegistry.class);
    ...
    public DefaultSubCrawlerRegistry(InputStream stream) throws IOException {
        parse(stream);
    }
    ...
    private void parse(InputStream stream) throws IOException {
        try {
            ...
            parser.setListener(new SubCrawlerParser());
            parser.parse(stream);
        }
    ...
    private class SubCrawlerParser extends SimpleSAXAdapter {
        ...
        public void startTag(String tagName, Map atts, String text) throws SAXException {
            if (SUB_CRAWLER_FACTORY_TAG.equals(tagName)) {
        ...
        private void processClassName(String className) {
            ...
            if (!className.equals("")) {
                try {
                    ...
                    SubCrawlerFactory factory = (SubCrawlerFactory) instance;
                    add(factory);
                }
